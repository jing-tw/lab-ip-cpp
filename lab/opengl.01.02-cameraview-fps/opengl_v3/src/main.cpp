...
int main()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        if (fDelta > 0 && fDistance >= 100.0f) {
            fDelta = -0.1f;
        }

        if (fDelta < 0 && fDistance <= 3.0f) {
            fDelta = +0.1f;
        }
        

        fDistance = fDistance + fDelta;
        printf("%f\n", fDistance);
        // Create view matrix using glm's lookAt function
        glm::mat4 view = glm::lookAt(glm::vec3(3.0f, 3.0f, fDistance), // Camera position (eye)
        ...
        nbFrames++;
        if (currentTime - lastTime >= 10.0) { // If lastTime is more than 1 sec ago
