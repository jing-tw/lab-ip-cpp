...
std::vector<byte_track::BYTETracker::STrackPtr> byte_track::BYTETracker::update(const std::vector<Object>& objects)
{
    ...
    std::cout << "byte_track:: objects.size() = " << objects.size() << std::endl;

    for (const auto &object : objects)
    {
        // debug
            std::cout << "byte_track:: update. rect.x() = " << object.rect.x() << std::endl;
         // end of debug
        ...
        if (object.prob >= track_thresh_)
        {
            ...
            std::cout << "1. det_stracks.push_back(strack); rect.x() = " << object.rect.x() << " track_thresh_ = " << track_thresh_ << " object.prob = " << object.prob << std::endl;
        }
        else
        {
            det_low_stracks.push_back(strack);
            std::cout << "2. det_low_stracks.push_back(strack);" << std::endl;
        }
    }
...

    {
        ...
        std::cout << "matches_idx.size() " << matches_idx.size() << std::endl;  // should 0 at firt
        std::cout << "unmatch_track_idx.size() " << unmatch_track_idx.size() << std::endl;  // should all in unmatch at firt
        std::cout << "unmatch_detection_idx.size() " << unmatch_detection_idx.size() << std::endl;
        ...
        {
            std::cout << "remain_det_stracks.push_back" << std::endl;
            remain_det_stracks.push_back(det_stracks[unmatch_idx]);
        }

        std::cout << "(should have value) remain_det_stracks.size() " << remain_det_stracks.size() << std::endl;  
...

    {
        ...
        /*
        non_active_stracks.size()=0
 remain_det_stracks.size()=1
 matches_idx.size()=0
 unmatch_unconfirmed_idx.size()=0
 unmatch_detection_idx.size()=1
        */
        std::cout   << "non_active_stracks.size()=" << non_active_stracks.size()  << std::endl\
                    << " remain_det_stracks.size()=" << remain_det_stracks.size() << std::endl \
                    << " matches_idx.size()=" << matches_idx.size() << std::endl \
                    << " unmatch_unconfirmed_idx.size()=" << unmatch_unconfirmed_idx.size() << std::endl \
                    << " unmatch_detection_idx.size()=" << unmatch_detection_idx.size() << std::endl  \
                    ;
        ...
        for (const auto &unmatch_idx : unmatch_detection_idx)
        {
            ...
            std::cout   << "Add new stracks: " << std::endl \
                        << " track->getScore() = " << track->getScore() << std::endl \
                        << " high_thresh_ = " << high_thresh_ << std::endl \
                        << std::endl;
            /*
            track->getScore() = 0.9
            high_thresh_ = 30
    ...
    ////////////////// Step 5: Update state //////////////////
    std::cout << "current_tracked_stracks.size() " << current_tracked_stracks.size() << std::endl;
